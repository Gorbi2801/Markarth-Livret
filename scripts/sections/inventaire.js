    ...
    return `<div style="background:var(--parch);border:1px solid var(--border-g);border-left:4px solid ${done?'#2D6A2D':'var(--gold)'};padding:0.7rem 1rem;">
      ...
      <div style="display:flex;align-items:center;gap:0.6rem;flex-wrap:wrap;">
        ...
        <div style="flex:1;min-width:80px;height:8px;background:var(--parch-dark);border:1px solid var(--border-g);overflow:hidden;">
          <div style="height:100%;width:${pct}%;background:${done?'#2D6A2D':'var(--gold)'};"></div>
        </div>
...
function renderInventaire(rows){
  invRows=rows;
  const tbody=document.getElementById('inv-tbody');const canAdmin=canEditSection('inventaire');
  const empty=rows.filter(r=>r.quantite<=0).length;
  document.getElementById('inv-total').textContent=rows.length;document.getElementById('inv-empty').textContent=empty;document.getElementById('inv-act-head').style.display=canAdmin?'':'none';
  tbody.innerHTML=rows.map(r=>`<tr data-search="${esc(r.nom.toLowerCase())}" data-stock="${r.quantite<=0?'vide':'dispo'}" data-cat="${esc(r.categorie||'')}">
    ...
    <td>${canAdmin?`<div style="display:flex;align-items:center;gap:0.5rem;"><button class="btn-del" style="border-color:var(--border-g);color:var(--ink-mid);font-size:1rem;width:26px;height:26px;display:flex;align-items:center;justify-content:center;padding:0;" onclick="updateQty('${r.id}','${escJs(r.nom)}',${r.quantite},-1)">−</button><input type="number" min="0" value="${r.quantite}" style="width:3.4rem;text-align:center;font-family:'Eagle Lake',serif;font-size:1rem;background:var(--parch);border:1px solid var(--border-g);color:${r.quantite<=0?'#7A1010':'var(--green-dark)'};padding:.15rem 0;" onchange="setQty('${r.id}','${escJs(r.nom)}',${r.quantite},this.value)"><button class="btn-del" style="border-color:var(--border-g);color:var(--ink-mid);font-size:1rem;width:26px;height:26px;display:flex;align-items:center;justify-content:center;padding:0;" onclick="updateQty('${r.id}','${escJs(r.nom)}',${r.quantite},1)">+</button></div>`:`<span style="font-family:'Eagle Lake',serif;font-size:1rem;color:${r.quantite<=0?'#7A1010':'var(--green-dark)'};">${r.quantite}</span>`}</td>
    ${canAdmin?`<td class="act"><button class="btn-del" onclick="editInvItem('${r.id}')">Modifier</button> <button class="btn-del" onclick="delInvItem('${r.id}')">Suppr.</button></td>`:''}</tr>`).join('');
}
...
async function addInvItem(){
  const nom=document.getElementById('inv-nom').value.trim();const categorie=document.getElementById('inv-categorie').value;const quantite=Math.max(0,parseInt(document.getElementById('inv-qty').value)||0);
  ...
  try{
    ...
    document.getElementById('inv-nom').value='';document.getElementById('inv-categorie').value='';document.getElementById('inv-qty').value='0';
...
async function applyQty(id,nom,current,newQty){
  ...
  try{await sbPatch('mk_inventaire',`?id=eq.${id}`,{quantite:newQty});const h=loadInvHistory();h.unshift({id,nom,oldQty:current,newQty,delta,player:session?session.username:'—',time:new Date().toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})});saveInvHistory(h);await loadInventaire();if(isLogged())renderInvHistory();}catch(e){console.error(e);toast('Erreur de mise à jour.');}
}
...
function renderInvHistory(){
  const list=document.getElementById('inv-history-list');const empty=document.getElementById('inv-history-empty');if(!list)return;
  const h=loadInvHistory();if(h.length===0){list.innerHTML='';empty.style.display='block';return;}
  ...
  list.innerHTML=h.slice(0,100).map((e,idx)=>`<div style="background:var(--parch);border:1px solid var(--border-g);border-left:3px solid ${e.delta>0?'var(--green-dark)':'#8B5E00'};padding:0.5rem 0.7rem;font-size:1rem;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.15rem;">
      ...
      <div style="display:flex;align-items:center;gap:0.4rem;"><span style="font-size:1rem;color:var(--ink-faint);font-style:italic;">${e.time}</span>
      <button onclick="revertInvEntry(${idx})" style="background:none;border:1px solid rgba(122,16,16,0.3);color:#7A1010;font-size:1rem;width:18px;height:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;" onmouseover="this.style.background='rgba(122,16,16,0.1)'" onmouseout="this.style.background='none'">✕</button></div></div>
    <span style="color:var(--ink-mid);">${e.oldQty} → ${e.newQty} <strong style="color:${e.delta>0?'var(--green-dark)':'#8B5E00'}">(${e.delta>0?'+':''}${e.delta})</strong></span></div>`).join('');
}
